...
from mlslicer.factgen import FactManager
...
class CodeTransformer(ast.NodeTransformer):
    def __init__(self, ignored_vars) -> None:
        ...
        self.subscript_sources = {}  # _var26 → (df_train, _var25)
    ...
    def handle_call_updates(self, call, call_saved, assigned_var=None):
         # ✅ Handle fillna / replace on a subscript or attribute (e.g., df["col"].fillna(...))
        if isinstance(call, ast.Call) and isinstance(call.func, ast.Attribute):
            if call.func.attr in ["fillna", "replace"]:
                obj = call.func.value  # e.g., _var26 or df_train
                value_expr = call.args[0] if call.args else ast.Constant(value="UNKNOWN")

                # 👉 For fillna: check ONLY for presence of the 'inplace' keyword (ignore its value)
                fillna_has_inplace_kwarg = (
                    call.func.attr == "fillna" and
                    any(kw.arg == "inplace" for kw in call.keywords)
                )
                replace_has_inplace_kwarg = (
                    call.func.attr == "replace" and
                    any(kw.arg == "inplace" for kw in call.keywords)
                )

                # 👉 For replace: always trigger
                should_trigger = (
                    fillna_has_inplace_kwarg or replace_has_inplace_kwarg
                )

                if should_trigger and isinstance(obj, ast.Name):
                    var_id = obj.id

                    if var_id in self.subscript_sources:
                        # Case: _var26 = df_train["col"]; _var26.fillna(..., inplace=True)
                        ...
                        # Return: df_train_0 = _var26.fillna(...)
    ...
    def handle_assign_value(self, target, value):
        ...
        # print("HANDLE ASSIGN VALUE", ast.dump(target),ast.dump(value))
        nodes = []
    ...
    def visit_Assign(self, node):
        nodes = []
        # print("VISIT ASSIGN",ast.dump(node))
    ...
    def visit_Subscript(self, node, assigned = False):
        nodes = []
        # print("VISIST SUBSCRIPT",ast.dump(node))
        nodes1, new_v = self.visitNameOnly(node.value)
        # print("VISIST SUBSCRIPT NEW NODE",[ast.dump(node) for node in nodes1])
        # print("VISIST SUBSCRIPT NEW VAL",ast.dump(new_v))
        if type(node.slice) == ast.Slice:
            ...
            assert 0
        # print("VISIST NEW SUBSCRIPT",ast.dump(ast.Subscript(new_v, slice)))
        # print("VISIST NEW STATEMENT", [ast.dump(node) for node in nodes1],[ast.dump(node) for node in nodes2], ast.dump(ast.Subscript(new_v, slice)))
    ...
    def visit_arguments(self, args):
        ...
        for i, arg in enumerate(args):
            ...
            # print(f"{ast.dump(arg)}---->{ast.dump(new_arg)} ")
            nodes += nodes1
    ...
    def visitNameOnly(self, node):
        ...
        if type(newNode) != ast.Name:
            ...
            # print("VISIT NAME ONLY", [ast.dump(node) for node in nodes1],ast.dump(new_var), ast.dump(newNode))
            if isinstance(newNode, ast.Subscript):
                if isinstance(newNode.value, ast.Name) and isinstance(newNode.slice, ast.Index):
                    base = newNode.value.id     # e.g., df_train
                    field = newNode.slice.value.id    # e.g., _var25
                    target = new_var.id  # _var26
                    self.subscript_sources[target] = (base, field)
